...
- 10.06.2026 вечер: ЖИВОЙ ТЕСТ БОТА пользователем (включая --real по его
  команде; для --real демо пересоздано под боевым NAVBAT_ENC_KEY — ключ
  дописан в ЛОКАЛЬНЫЙ .env Windows-машины, на другой машине нужен свой
  .env или копия; два процесса бота одновременно = 409 Conflict).
  Находки теста → ОДОБРЕННЫЙ батч «полировка диалога + витрина»
  (следующий шаг, начать со спеки brainstorm):
  1) вопрос о наличии в любых формулировках («а больше слотов нету?»,
     «а ещё?», «другой день?») → слоты/«Другое время», БЕЗ эскалации
     (сейчас уходит в «вне компетенции» → дёргает админа по пустяку);
  2) смягчение эскалаций: непонятый текст → «не понял» + меню (счётчик
     сбрасывается при использовании меню); эскалация только по прямой
     просьбе человека ИЛИ при сбоях посреди оформления записи;
  3) запрошенное время вне рабочего окна («а в 21 могу?») → явная строка
     «клиника работает до 18:00» ПЕРЕД слотами;
  4) инлайн-календарь выбора даты: НЕ замена happy path (ближайшие слоты
     кнопками остаются), а углубление за «Другое время» → «📅 Выбрать
     дату»: сетка месяца на callback-кнопках, кликабельны ТОЛЬКО дни
     со свободными слотами, навигация по месяцам, ru/uz. ВИЗУАЛ —
     ПРИОРИТЕТ пользователя (явное требование): выжать максимум —
     эмодзи-маркеры, компактная сетка, опрятная типографика.
     Mobiscroll/Mini App ОТКЛОНЕНЫ для v1.0 (платная лицензия + веб-
     фронтенд + хостинг); «booking как Mini App» — кандидат Ф3;
  5) из «витрины»: сценарий демо-показа продажи + /stats глазами
     владельца клиники.
- Рабочие заметки: Docker Desktop может быть выключен — стартануть и
  ждать демона; deploy/.env сгенерирован локально (gitignored);
