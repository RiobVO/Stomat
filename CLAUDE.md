...
- P3 РЕФАКТОРИНГ ЯДРА В РАБОТЕ (решение пользователя «весь P3 по порядку»,
  ...
  (88e314b); R1c appointments_repo ГОТОВ (5c1b1f7). R1 закрыт целиком:
  fsm.py больше не держит сырого SQL. R2 типизированное состояние ГОТОВ
  (3dc1ba8 — DialogContext dataclass вместо conv.context dict, +extras
  для adapter-ключей tg_actions; _BOOKING_KEYS/_PII_CONTEXT_KEYS сняты).
  Тестов 525, всё в origin/master.
- СЛЕДУЮЩИЙ ШАГ: последний кусок P3 — R4 (разбить god-object DialogEngine
  ~820 строк по сценариям booking/reschedule/cancel за общим роутером).
  Это самый крупный и рискованный кусок: чистое перемещение методов, без
  смены поведения, страховка — 525 тестов + серия ×8. Перед кодом —
  brainstorm границ модулей (что в роутер, что в сценарий, общие хелперы).
  План — разд. P3 docs/PILOT_HARDENING.md. Цикл: правка → сьют → серия ×8
  → демо восстановить → коммит.
