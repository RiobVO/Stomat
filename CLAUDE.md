...
- 11.06.2026 БАТЧ «ПОЛИРОВКА ДИАЛОГА + ВИТРИНА» ЗАКРЫТ ЦЕЛИКОМ
  ...
  П-5 (ed5f19f): инлайн-календарь за «📅 Выбрать дату» — визуал «максимум
  эмодзи» (выбор пользователя: 🟢 день со слотами, 📍 сегодня, пустые
  некликабельны), навигация текущий+2 месяца edit'ом, day-view ВСЕХ
  слотов дня сеткой, ru/uz (calendar_view.py чистый + _CalendarFlowMixin);
  «нет слотов 2 недели» → календарь пациенту + 🟡 FYI раз в день
  (in-memory дедуп), НЕ «передаю администратору»;
  ...
  Тестов ~758. m2 (NLU-сигнал наличия) остаётся на пилот — словари
  прикрывают, живые данные покажут.
