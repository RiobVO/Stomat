...
- Остановились 07.06.2026: кодовая часть Ф1.5 закрыта и запушена
  (последний коммит f31d6eb), 308 тестов зелёные. Живой смок демо-бота
  ОТЛОЖЕН решением пользователя — не предлагать.
- СЛЕДУЮЩИЙ ШАГ (делать без напоминания): docs/PRIVACY.md — карта
  персональных данных для юриста по ПД РУз (подготовка D.1, единственный
  блокер пилота). Только инженерные факты, сверенные с кодом: что собираем
  (имя, телефон, tg_chat_id, тексты), как храним (AES-256-GCM имена,
  SHA-256+salt телефоны, RLS), что уходит в US-LLM (деидентифицированный
  текст — nlu/wrappers.py), права субъекта (/forget), retention 90 дней,
  что НЕ собираем. Юридические выводы НЕ делать — их делает юрист.
  Коммит docs(privacy) + push. Plan mode не нужен (документ).
- После PRIVACY.md — стоп: ждать вход пользователя (итоги юриста по D.1,
  носитель для D.4). Можно предложить ровно одно: выгрузку всех узбекских
  строк replies.py в один файл для проверки носителем (подготовка D.4).
...
- Ф1.5 Production-Ready v1.0: КОДОВАЯ ЧАСТЬ ЗАКРЫТА 07.06.2026 — группы A
  ...
  Детали — чекбоксы BRIEF разд. 14. Осталось ВНЕШНЕЕ: D.1 юрист по ПД РУз
  (шаг пользователя, BLOCKING для пилота), D.4 узбекские строки replies.py
  носителем, группа C — строго по явной команде пользователя.
