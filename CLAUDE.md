...
- Остановились 07.06.2026: кодовая часть Ф1.5 закрыта и запушена, 308
  ...
  D.4 ЗАКРЫТ (коммит 0b7815a): узбекские строки проверены кросс-ревью
  LLM вместо носителя (решение пользователя), 16 правок внесены через
  TDD (tests/test_replies_uz.py пинит тексты + инварианты шаблонов),
  сьют 8/8 зелёный (495 тестов), демо восстановлено, --check все [OK].
  Решения раунда — шапка docs/UZ_STRINGS.md. Живой смок демо-бота
  ОТЛОЖЕН решением пользователя — не предлагать.
- СЛЕДУЮЩИЙ ШАГ: нет — весь чеклист v1.0 закрыт, кроме группы C
  (строго по явной команде пользователя). Стоп.
- Дальше только по входу пользователя: пилот Ф2, группа C, платные
  прогоны. Известная мелочь в бэклоге: python -m navbat.telegram --real
  собирает экстрактор без DeidentifyingExtractor (telegram/app.py:65-72)
  — фикс по команде.
