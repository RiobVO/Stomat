...
- Ф1: все 5 инкрементов кодового ядра — ЗАКРЫТЫ. Система запускается одной
  командой `python -m navbat` с локальной машины (решение: всё локально,
  деплой на VPS — после первой продажи).
- Текущий этап: полировка демо по живым прогонам (docs/DEMO.md) + поиск пилотов.
...
- LLM-fallback на второго провайдера — перед продом (решение: отложен,
  качество на узбекском не проверено; сейчас таймаут 8с + эскалация).
- Деплой (VPS, nginx/certbot, webhook, watch) — после первой продажи.
