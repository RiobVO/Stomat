...
- P3 РЕФАКТОРИНГ ЯДРА ЗАКРЫТ ЦЕЛИКОМ (порядок по риску R3→R1→R2→R4):
  ...
  dialog_common (b00f89c). Тестов 525, всё в origin/master.
- CI был красным всю сессию по НЕ связанной с P3 причине: тест
  test_telegram_real_path зависел от ambient OPENAI_API_KEY (локально есть,
  в GitHub нет → sys.exit). Поправлен dummy-ключом через monkeypatch
  (c5edd33). После пуша проверить, что Actions позеленел.
- СЛЕДУЮЩИЙ ШАГ: P3 завершён. Из открытого в PILOT_HARDENING остались
  только P4 (T1 NLU-регрессия, T2 Gemini-failover — оба упираются в
  деньги/решение пользователя, не код) и отложенные m2/m3/m4. Новых
  обязательных кусков по P3 нет — ждать явной команды пользователя
  (новая задача / пилот Ф2 / группа C).
