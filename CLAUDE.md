...
- Остановились 07.06.2026: кодовая часть Ф1.5 закрыта и запушена, 308
  ...
  Живой смок демо-бота ОТЛОЖЕН решением пользователя — не предлагать.
- СЛЕДУЮЩИЙ ШАГ (делать без напоминания): подготовка D.4 — выгрузить все
  узбекские строки replies.py (TEMPLATES uz + SERVICE_LABELS uz +
  MEDICAL_DISCLAIMER uz) в один файл docs/UZ_STRINGS.md в формате
  «ключ | русский | узбекский | контекст показа» — чтобы носитель языка
  правил в одном месте, не лазая по коду. Коммит docs(i18n) + push.
  Plan mode не нужен (выгрузка). После неё — стоп.
- Дальше только по входу пользователя: итоги юриста (D.1), правки носителя
  ...
  и группа C — строго по явной команде.
