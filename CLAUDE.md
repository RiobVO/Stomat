...
- Группа E — ЗАКРЫТА 07.06.2026: дашборд денег (отмена из напоминания =
  actor='reminder' в аудите; prevented/saved в /stats и дайджесте),
  onboard --import-calendar, чеклист 5 тест-диалогов в docs/DEMO.md;
  приёмочные §6 — оба «недостающих» теста существовали (аудит ошибся).
- Ф1.5 КОДОВАЯ ЧАСТЬ ЗАКРЫТА (A, B, D.2-D.3, E — 308 тестов). Остались
  внешние: D.1 юрист по ПД (пользователь, BLOCKING для пилота), D.4 узбекские
  строки носителем, группа C (строго по явной команде пользователя).
