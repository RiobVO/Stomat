...
- Остановились 07.06.2026: кодовая часть Ф1.5 закрыта и запушена, 308
  тестов зелёные. docs/PRIVACY.md (карта ПД для юриста, D.1) — ГОТОВ
  (коммит deaaa2f): пользователь несёт его юристу, бот-часть D.1 сделана.
  docs/UZ_STRINGS.md (все 67 uz/ru-пар replies.py для носителя, D.4) —
  ГОТОВ (коммиты 33ed5d9 + d494704): пользователь несёт его носителю;
  внутри примеры подстановок и вопросы носителю (апостроф vs okina,
  аффиксы вокруг дат, термины). Живой смок демо-бота ОТЛОЖЕН решением
  пользователя — не предлагать.
- СЛЕДУЮЩИЙ ШАГ: нет — обе подготовки внешних пунктов сделаны, стоп.
- Дальше только по входу пользователя: итоги юриста (D.1), правки носителя
  (D.4 — внести обратно в replies.py через TDD), пилот Ф2. Платные прогоны
  и группа C — строго по явной команде. Известная мелочь в бэклоге:
  python -m navbat.telegram --real собирает экстрактор без
  DeidentifyingExtractor (telegram/app.py:65-72) — фикс по команде.
