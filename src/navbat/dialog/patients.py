...
def phone_to_hash(session: Session, raw_phone: str) -> str:
    """Сырой номер → SHA-256-хэш (нормализация E.164 + соль клиники).

    ValueError, если строка не похожа на номер (не 7–15 цифр, П-2в) —
    вызывающий решает, что делать (на границе enqueue хэш не кладётся,
    диалог повторяет кнопку контакта).
    """
