...
def _known_names(session) -> set[str]:
    ...
    Страховка для строк очереди, попавших туда ДО того, как имя стали
    стирать в источнике (dialog/booking_flow.py зовёт queue.redact_message_
    text сразу, как распознал имя). Основную границу держит она — здесь
    закрываются старые payload'ы.

    Сверка точная, не эвристика: это ровно тот же текст, который стал именем
    — в patient (сценарий дошёл до телефона) или в pending_name незакрытого
    диалога. Ошибка расшифровки не глушится: корпус с возможным PII лучше не
    создать вовсе, чем создать наполовину проверенным.
    """
    names = {
