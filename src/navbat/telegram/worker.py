...
# записи из календарного синка: тот же долгий срок жизни. rcl: — приглашение
# на повторный визит: висит в чате днями, пока пациент соберётся. rate: —
# звёзды под просьбой об оценке: тот же срок жизни, субъект (приём) в кнопке.
RAW_CALLBACK_PREFIXES = ("cal:", "wl:", "unfreeze", "attend:", "remind_cancel:",
                         "reslot:", "time:", "d:", "rcl:", "rate:")
        ...
        if "message" in payload:
            ...
            if "text" in message:
                ...
                if (chat_id in self._admin_chat_ids
                    ...
                    reply = self._relay_reply(chat_id, message)
                    try:
                        self._send(chat_id, reply)
                    except TelegramAPIError as e:
                        # подтверждение админу — best effort: текст пациенту
                        # УЖЕ отправлен внутри _relay_reply, а доставка не
                        # идемпотентна. Пустить исключение выше — значит
                        # вернуть апдейт в pending, и ретрай пошлёт пациенту
                        # вторую копию (до трёх на трёх попытках). Потерянная
                        # галочка админу — включая ChatUnavailableError, когда
                        # недоступен сам админ-чат, — дешевле дубля у пациента.
                        log.error("подтверждение свайп-ответа админу %s "
                                  "не доставлено: %s", chat_id, e)
                    return
                if chat_id in self._admin_chat_ids:
