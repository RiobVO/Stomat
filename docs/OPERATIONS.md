...
| «Системный алерт: бэкапы БД не снимаются» | `docker compose logs backup`; контейнер мёртв → `up -d backup` |

## Ротация ключа шифрования (NAVBAT_ENC_KEY)

Когда: ключ скомпрометирован — утёк `.env`, бэкап вместе с ключом, ушёл
человек, имевший доступ. Ключом шифрованы имена пациентов/врачей, токены
ботов, webhook-секреты и Google-токены — ротация перешифровывает всё.

```bash
docker compose -f docker-compose.prod.yml stop app        # 1. остановить бота
python -c "import os,base64; print(base64.b64encode(os.urandom(32)).decode())"
NAVBAT_ENC_KEY_NEW=<новый ключ> python -m navbat.rotate_key   # 2. перешифровка
# 3. в deploy/.env заменить NAVBAT_ENC_KEY на новый, копию — вне сервера
docker compose -f docker-compose.prod.yml up -d app       # 4. поднять
```

Скрипт идёт одной транзакцией: сбой на любом значении — база не изменена.
Повторный прогон безопасен (уже перешифрованное пропускается). В контейнере:
`docker compose run --rm -e NAVBAT_ENC_KEY_NEW=<новый> --entrypoint python
app -m navbat.rotate_key`.
...
**Учения раз в месяц:** прогнать процедуру ниже на локальной машине
(не на проде) — бэкап, который ни разу не разворачивали, не считается
бэкапом. Свежесть бэкапов между учениями сторожит /health (алерт
владельцу при простое sidecar'а).
